...
const loadCreateNewProject = function (
  contentSection,
  myProject,
  mySavedProject
) {
...
function renderProject(contentSection, myProject, mySavedProject) {
  myProject.forEach((element) => render(element));
...
function removeItem(button, myProject, mySavedProject, contentSection) {
  for (let i = myProject.length - 1; i >= 0; i--) {
    if (myProject[i] === button.value) myProject.splice(i, 1);
  }

  myProject.forEach((element) => (mySavedProject = element));
